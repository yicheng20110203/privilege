<template>
  ...
  >
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <el-image :key="scope.row.id" style="width: 80%; height: 40%" :src="scope.row.entity_id" lazy>
...
import { TYPE_PICTURE, MATERIAL_PICTURE } from '@/utils/global-element'
export default {
  name: 'SelectMaterialPicture',
  data() {
    return {
      visible: false,
      is_popover: true,
  ...
  methods: {
    ...
    dataFormSubmit() {
      this.visible = false
      this.$emit('refreshPictureDataList', this.raidioPictureList)
    },
    // 每页数
    ...
    // 选择素材触发
    radioChange(data) {
      this.raidioPictureList = data.row
    },
    // 选择素材图片分类
    handleChange(value) {
      // 选中获取分类
