  ...
  <div class="dashboard-container">
    <el-scrollbar>
      <el-form ref="dataForm" :rules="dataRule" :model="dataForm" label-width="auto" size="medium" label-suffix=":">
        ...
        <el-form-item label="选择音频" prop="materialName">
          <el-button type="primary" style="margin-bottom:5px;" @click="selectVideoHandle">+选择音频</el-button>
          <!-- <el-tag v-if="dataForm.materialName" type="success">{{ dataForm.materialName }}</el-tag> -->
          <el-table
            v-if="audioSelect.length !== 0"
...
import { ISDELETE, TYPE_AUDIO, SECTION_AUDIO, MATERIAL_AUDIO } from '@/utils/global-element'
  ...
  data() {
    return {
      ...
      dataRule: {
        ...
        classification: [{ required: true, message: '音频分类不能为空', trigger: 'blur' }],
        materialName: [{ required: true, message: '请选择音频', trigger: 'blur' }],
        imageUrl: [{ required: true, message: '请上传音频封面', trigger: 'blur' }]
      },
  ...
  methods: {
          ...
          contentAdd(this.$service.adornData({
            ...
            if (res && res.code === 0) {
              this.$message({
                ...
                duration: 1500,
                onClose: () => {
                  this.$router.replace({ path: '/course/content-management?activeName=audio' })
                }
              })
            } else {
    ...
    getmaterialAudioDataList(type = {}) {
      getMaterialList(this.$service.adornData({
        ...
        this.materialAudioDataList = res && res.code === 0 ? res.data.list : []
        this.totalPage = res && res.code === 0 ? res.data.total_size : []
      })
    },
    // 通过id获取素材-音频
    getmaterialAudio(type = {}) {
      getMaterialList(this.$service.adornData({
        'page': this.pageIndex,
        'size': this.pageSize,
        'name': this.selectDataForm.name,
        ...
        this.audioSelect = res && res.code === 0 ? res.data.list : []
      })
    },
    // 素材-音频-查询
    selectMaterialVideoHandle() {
      this.getmaterialAudioDataList({
        'type': TYPE_AUDIO,
    ...
    getMaterialVideoUpdate() {
      getContentList(this.$service.adornData({
        ...
        if (res && res.code === 0) {
          ...
          this.getmaterialAudio({ 'type': TYPE_AUDIO, 'id': this.materialId, 'name': this.dataForm.materialName })
        } else {
          this.$message.error(res.msg)
        }
      })
